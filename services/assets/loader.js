...
class Loader {
  ...
  getFiles(group = null) {
    ...
    this.loadedFiles.forEach((file) => {
      if (group === null || file.groups.includes(group)) {
  ...
  get(group = null, synchronous = false) {
    ...
    return new Promise((resolve) => {
      ...
      this.events.on(`load:${group}`, (results) => {
        resolve(results)
      })
    })
  }
  ...
  onLoad(file, result) {
    ...
    file.groups.forEach((group) => {
      if (this.isLoaded(group)) {
        this.events.emit(`load:${group}`, this.getFiles(group))
      }
    })

    if (this.isLoaded()) {
      this.events.emit('load', this.getFiles())
    }
  }
  ...
  addFile(file, inheritanceGroup = []) {
    const finalFile = file.name
      ? file
      : { name: file, path: file, datas: file.datas }
  ...
  loadFile(file) {
    ...
    loader.load(
      ...
      (e) => {
        ...
        if (this.verbose)
          console.error(
            `Loader: Cannot load file "${file.name}" with path "${file.path}"`
          )
      },
